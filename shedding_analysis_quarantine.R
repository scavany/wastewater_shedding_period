...
for (delay.mean in delay.means) {
    ...
    ## Open figure 1
    pdf(paste0("Fig1_",delay.mean,".pdf"),height=10,width=10)
...
##Do the convolution for the expectation
    ...
    NLL.dispersion <- function(par) {
        ...
        for (jj in 1:(length(n.exp)-1)) {
            ...
            individual.shedding.symp <- individual.shedding * (1 - p.enter + p.exit)
            individual.shedding.asymp <- individual.shedding * (1 - p.enter.asymp + p.exit.asymp)
    ...
    lower <- c(logshape=log(1e-2),lograte=log(1e-2),logshedscale=log(1e-1), lognbinomsize=log(1e-3))#confint(mle2.profile)[,1]*0.1
    upper <- c(logshape=log(1e5),lograte=log(1e7),logshedscale=log(1e3), lognbinomsize=log(1e1))#confint(mle2.profile)[,2]*1.9
    bayesianSetup = createBayesianSetup(LL.dispersion,lower=lower,upper=upper)
    mcmc.out = runMCMC(bayesianSetup)
    pdf(paste0("mcmc_out_quarantine_",delay.mean,".pdf"))
    plot(mcmc.out,start = 1000)
    dev.off()
    samples = getSample(mcmc.out,start=1000)
    save(mcmc.out,samples,file=paste0("mcmc_quarantine_delay_pois",floor(1/delay.rate+0.5),".RData"))
    load(paste0("mcmc_quarantine_delay_pois",floor(1/delay.rate+0.5),".RData"))
    ...
    pdf(paste0("Fig2_quarantine_",delay.mean,".pdf"),width=10,height=10)
    ...
    plot(0:30, colMeans(shed.dist) * (1 - p.enter + p.exit),
         type='l', ylim=c(0,max(apply(shed.dist[,],2,function(x)quantile(x,0.975)))),
         ylab="Shedding intensity (N1 GC / 100ml)",xlab="Time since infection",yaxs="i",las=1,bty="n",
         col="red",lwd=2)
    lines(0:30,colMeans(shed.dist),lwd=2)
    mtext("A",side=3,line=0, 
          at=par("usr")[1]+0.05*diff(par("usr")[1:2]),
          cex=1.2)
    ...
    abline(v=which.max(colMeans(shed.dist))-1,lty="dashed")
    shed.conc.max <- colMeans(shed.dist)[which.max(colMeans(shed.dist))] 
    ...
    mtext("B",side=3,line=0, 
          ...
          cex=1.2)
    ##dev.off()
    (0:30)[which.max(colMeans(shed.dist))]
    ...
    plot(date.exp,colMeans(pred),type='l',xlim=c(min(ww.data$date),max(ww.data$date)),
         ylim=c(0,max(ww.data[,2:4], upper95)),
...
## }
## lines(date.exp,colMeans(pred,na.rm=TRUE),col="red",lwd=2)
## plotCI(ww.data$date,N1.mean,li=N1.min,ui=N1.max,add=TRUE,gap=TRUE,sfrac=0.003)
## dev.off()
