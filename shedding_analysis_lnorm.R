...
p_load(EpiEstim, tidyverse, incidence, plotrix, distr, data.table,
...
pdf("incidence_curve.pdf")
plot(incid,border="white") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
Inc.asymp <- Weibull(shape=inc.shape,scale=inc.scale/2)
...
mu <- matrix(0,ncol=ncol(sts.symp.bp),nrow=nrow(sts.symp.bp))
...
for (j in 1:ncol(sts.symp.bp)) { 
  ...
  for (t in 1:nrow(sts.symp.bp)) {
    ...
    i <- seq_len(t)
    mu[t,j] <- sum(inc.symp.pmf[t-i+1] * upperbound(sts.symp.bp)[i,j],na.rm=TRUE)
  }
}
#Show the fit
lines(1:nrow(sts.symp.bp)-0.5,mu[,1],col="green",lwd=3,lty="dashed")
legend("topleft", lty=c(NA,"solid","dashed"),col=c(NA,"blue","green"),fill=c("white",NA,NA),
       legend=c("Cases","Infections","Predicted Cases"),bty="n",border=c("black",NA,NA))
plot(sts.asymp.bp,xaxis.labelFormat=NULL,legend=NULL,lwd=c(1,1,2),lty=c(1,1,1),main="",
     xaxs="i",bty="n",yaxs="i",las=1)
#Do the convolution for the expectation
mu <- matrix(0,ncol=ncol(sts.asymp.bp),nrow=nrow(sts.asymp.bp))
#Loop over all series
for (j in 1:ncol(sts.asymp.bp)) { 
  ...
  for (t in 1:nrow(sts.asymp.bp)) {
    #Convolution, note support of inc.pmf starts at zero (move idx by 1)
    i <- seq_len(t)
    mu[t,j] <- sum(inc.asymp.pmf[t-i+1] * upperbound(sts.asymp.bp)[i,j],na.rm=TRUE)
  }
}
#Show the fit
...
plotCI(ww.data$date,N1.mean,li=N1.min,ui=N1.max,gap=TRUE,sfrac=0.003,
...
NLL.dispersion <- function(par) {
    ...
    lnorm.sdlog <- exp(par[4])
    if (sum(shedding.scaled)) {
        return(-sum(dlnorm(unlist(ww.data[ww.data$date %in% date.exp,2]),
                           meanlog=log(shedding.scaled[date.exp %in% ww.data$date]) - lnorm.sdlog^2/2,
                           sdlog=lnorm.sdlog,log=TRUE)) -
               sum(dlnorm(unlist(ww.data[ww.data$date %in% date.exp,3]),
                          meanlog=log(shedding.scaled[date.exp %in% ww.data$date]) - lnorm.sdlog^2/2,
                          sdlog=lnorm.sdlog,log=TRUE)) - 
               sum(dlnorm(unlist(ww.data[ww.data$date %in% date.exp,4]),
                          meanlog=log(shedding.scaled[date.exp %in% ww.data$date]) - lnorm.sdlog^2/2,
                          sdlog=lnorm.sdlog,log=TRUE)))
    } else {
        return(Inf)
...
NLL.mle2.dispersion <- function(logmean=1,logsd=1,logshedscale=1,lognbinomsize=1) {
    return(NLL.dispersion(c(logmean,logsd,logshedscale,lognbinomsize)))
}
...
lower <- c(logmean=log(1e0),logsd=log(1e0),logshedscale=log(1e-1), loglnormsdlog=log(1e-3))#confint(mle2.profile)[,1]*0.1
upper <- c(logmean=log(1e5),logsd=log(1e7),logshedscale=log(1e3), loglnormsdlog=log(1e1))#confint(mle2.profile)[,2]*1.9
bayesianSetup = createBayesianSetup(LL.dispersion,lower=lower,upper=upper)
mcmc.out = runMCMC(bayesianSetup)
pdf("mcmc_out_lnorm.pdf")
plot(mcmc.out,start = 500)
dev.off()
samples = getSample(mcmc.out,start=500)
save(mcmc.out,samples,file=paste0("mcmc_lnorm_delay_pois",floor(1/delay.rate+0.5),".RData"))
...
shed.conc.7 <- colMeans(shed.dist)[8] # mean concentration at 7 days (incubation period)
barplot(dnbinom(0:10,size=mean(nbinom.size),mu=shed.conc.7),ylab="Proportion of individuals",
        names.arg=0:10,
        xlab="Shedding intensity (N1 GC / 100ml)")
dev.off()
...
plotCI(ww.data$date,N1.mean,li=N1.min,ui=N1.max,add=TRUE,gap=TRUE,sfrac=0.003)
polygon(c(date.exp,rev(date.exp)),c(apply(pred,2,function(x)quantile(x,0.025)),
                                    rev(apply(pred,2,function(x)quantile(x,0.975)))),
        col=adjustcolor("gray",0.5),border=F)
dev.off()

pdf(paste0("shedding_timeseries_",floor(1/delay.rate+0.5),"_mean_delay_spaghettid.pdf"))
