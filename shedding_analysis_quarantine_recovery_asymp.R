...
##Do the convolution for the expectation
    ...
    NLL.dispersion <- function(par) {
        ...
        for (jj in 1:(length(n.exp)-1)) {
            ...
            individual.shedding.asymp <- individual.shedding * p.scale.asymp * (1 + p.q * (p.exit.asymp - p.enter.asymp))
    ...
    dteunis <- function(x,alpha,gamma) {
        (alpha/gamma)*(alpha+gamma)*(exp(-alpha*x) - exp(-(alpha+gamma)*x))
    }
    pteunis <- function(x,alpha,gamma) {
        1 + (1/gamma)*(alpha*exp(-(alpha+gamma)*x) - (alpha+gamma)*exp(-alpha*x))
    }
    ...
    for (jj in 1:length(shed.shape)) {
        for (ii in 1:(length(n.exp)-1)) {
            ...
            individual.shedding.asymp <- individual.shedding * p.inf.asymp[jj] * (1 + p.q[jj] * (p.exit.asymp - p.enter.asymp))
