...
### Plan for analyzing wastewater data
    ...
    save(dates,n.exp,file=paste0("nexp_delay_pois",floor(1/delay.rate+0.5),".RData"))
    ## pdf("infections.pdf")
    ## par(mfrow=c(2,1))
...
##Do the convolution for the expectation
    ...
    NLL.dispersion <- function(par) {
        ...
        for (jj in 1:(length(n.exp)-1)) {
            ...
            shedding <- shedding + n.exp[jj]*c(rep(0,jj-1),cum.dens.1-cum.dens.0)
        }
    ...
    pdf(paste0("mcmc_out_",delay.mean,".pdf"))
    if (delay.mean == 1) {
    ...
    pdf(paste0("Fig2_",delay.mean,".pdf"),width=10,height=10)
    ...
    barplot(dnbinom(0:10,size=mean(nbinom.size),mu=shed.conc.max),ylab="Proportion of individuals",
            names.arg=0:10,
